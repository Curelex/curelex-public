...
//       </p>
//       <div style={{ background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 10px', fontSize: 11, wordBreak: 'break-all', marginBottom: 10, color: 'var(--text-secondary)' }}>
//         {link}
//       </div>
...
// // ── Status Banner ─────────────────────────────────────────────────────────────
// function StatusBanner({ status, onAction, doctorName }) {
...
// function AvailabilityToggle({ isActive, onToggle, isMobile = false }) {
//   return (
//     <button
//       onClick={onToggle}
//       style={{
...
//           </div>
//           <div style={{ position: 'absolute', top: 0, left: 0, width: 52, height: 52, borderRadius: '50%', border: '2px solid rgba(255,255,255,0.4)', animation: 'ping 1.5s cubic-bezier(0,0,0.2,1) infinite' }} />
//         </div>

//         <div style={{ flex: 1, minWidth: 200 }}>
//           <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
//             <span style={{ background: '#fbbf24', color: '#78350f', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, textTransform: 'uppercase', letterSpacing: 0.5 }}>New Patient</span>
...
//             {patient.patient?.age && <span><i className="fas fa-birthday-cake" style={{ marginRight: 4 }}></i>{patient.patient.age} yrs</span>}
...
//             {(patient.type || patient.appointmentType) && <span><i className="fas fa-tag" style={{ marginRight: 4 }}></i>{patient.type || patient.appointmentType}</span>}
//           </div>
...
//               style={{ width: '100%', padding: '8px 14px', borderRadius: 8, border: '1.5px solid rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.15)', color: 'white', fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
...
//   // ── Active/Inactive state (persisted in localStorage)
//   const [isActive, setIsActive] = useState(() => {
//     return localStorage.getItem('doctor-is-active') !== 'false' // default true
//   })
...
//     showToast(next ? '✅ You are now Active — patients can book appointments' : '🔴 You are now Inactive — no new bookings will come in', next ? 'success' : 'info')
//   }

//   useEffect(() => {
//     if (!doctor) { navigate('/'); return }
//     // Load localStorage photo
...
//         const isComplete = !!(doctorData?.specialization && doctorData?.experience && doctorData?.regNumber)
...
//       setProfile(storedData ? JSON.parse(storedData) : doctor)
//     }
//   }
...
//       <style>{`
//         /* Closed-shop fade overlay */
//         .dd-content-area {
//           position: relative;
//           transition: opacity 0.4s ease;
//         }
//         .dd-content-area.inactive {
//           opacity: 0.38;
//           pointer-events: none;
//           filter: grayscale(60%);
//         }
...
//         /* Hide/show availability toggle by screen size */
//         .avail-desktop { display: flex; }
//         .avail-mobile  { display: none; }

//         @media (max-width: 640px) {
//           .avail-desktop { display: none !important; }
//           .avail-mobile  { display: flex !important; }
//           .dd-time-date  { display: none !important; }
//         }
//       `}</style>
...
//           border: '1px solid #f0f0f0', gap: 16, flexWrap: 'wrap', overflow: 'hidden',
//         }}>
//           {/* Left: greeting + name + info */}
...
//               Hi, Dr. {d?.name || 'Doctor'}
//             </h1>
//             <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center', fontSize: 13, color: '#6b7280' }}>
//               {d?.specialization && (
...
//               </span>
//               {/* Mobile: show toggle here instead of time */}
//               <span className="avail-mobile">
//                 <AvailabilityToggle isActive={isActive} onToggle={toggleActive} isMobile={true} />
//               </span>
//             </div>
//           </div>

//           {/* Middle: desktop availability toggle — always shown */}
//           <div className="avail-desktop" style={{ alignItems: 'center' }}>
//             <AvailabilityToggle isActive={isActive} onToggle={toggleActive} isMobile={false} />
//           </div>
...
//               {!d?.photo && initials}
//             </div>
//             {/* Online/offline dot follows isActive */}
//             <div style={{
...
//         {/* ── Status Banner (pending/incomplete) ── */}
//         {bannerStatus && (
//           <StatusBanner
//             status={bannerStatus}
...
//         {/* ── Dashboard Content with inactive overlay ── */}
//         {isContentVisible && (
//           <div className={`dd-content-area${isActive ? '' : ' inactive'}`}>
//             {/* Closed-shop badge when inactive */}
//             {!isActive && (
...
//                               </button>
//                             )}
//                             <button className="btn btn-outline btn-sm" onClick={() => setPrescriptionModal({ patientId: appt.patientId, appointmentId: appt.id })}>
...
//                       {d?.photo
//                         ? <img src={d.photo} alt="Doctor" style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover' }} />
...
/* ─── Doctor Nav Items (like patient sidebar) ───────────────── */
const NAV_ITEMS = [
      ...
      <div className="pd-below-header">
        <div className="pd-main" style={{ width: '100%' }}>
          <main className="pd-body">
            ...
            {/* Welcome Header */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'white', borderRadius: 16, padding: '20px 28px', boxShadow: '0 2px 12px rgba(0,0,0,0.07)', marginBottom: 24, border: '1px solid #f0f0f0', gap: 16, flexWrap: 'wrap' }}>
              <div style={{ flex: 1 }}>
                <p style={{ margin: 0, fontSize: 13, color: '#6b7280', fontWeight: 500 }}>{greeting} 👋</p>
                <h1 style={{ margin: '4px 0 8px', fontSize: 'clamp(20px,4vw,28px)', fontWeight: 700, color: '#111827' }}>
                  Hi, Dr. {d?.name || 'Doctor'}
              ...
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <div style={{ width: 72, height: 72, borderRadius: '50%', background: d?.photo ? 'transparent' : 'linear-gradient(135deg,#2563eb,#7c3aed)', backgroundImage: d?.photo ? `url(${d.photo})` : undefined, backgroundSize: 'cover', backgroundPosition: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 700, color: 'white', boxShadow: '0 4px 12px rgba(37,99,235,0.3)' }}>
                  {!d?.photo && initials}
                </div>
                <div style={{ position: 'absolute', bottom: 2, right: 2, width: 16, height: 16, borderRadius: '50%', background: isActive && isContentVisible ? '#10b981' : '#9ca3af', border: '2.5px solid white', transition: 'background 0.3s' }} />
              </div>
            </div>
